...
if(isset($_GET['action']))
{
	switch($_GET['action'])
	{
		...
		case 'extract':
			...
			break;
		case 'media_preview':
			$preview_file = $function_prefix == 's3_'
				? 'http://'.$kernel->conf['s3_domain'].'/'.$current_path . $_GET["file"]
				: $kernel->sets['paths']['server_url'] . str_replace('\\', '/', substr(realpath($current_path . $_GET["file"]), strlen($kernel->sets['paths']['doc_root'])));
		...
		case 'get_file': // preview or edit
			...
			{
				$preview_file = $function_prefix == 's3_'
					? 'http://'.$kernel->conf['s3_domain'].'/'.$selected_file
					: $kernel->sets['paths']['server_url'] . str_replace('\\', '/', substr(realpath($selected_file), strlen($kernel->sets['paths']['doc_root'])));
				if ($preview_mode == 'text')
				{
					// get and sanities
				...
				elseif ($preview_mode == 'viewerjs')
				{
					$ret = '<iframe id="viewer" src="js/ViewerJS/#'.$preview_file.'" allowfullscreen="" webkitallowfullscreen="" class="viewer-iframe"></iframe>';

				}
				elseif ($preview_mode == 'google')
				{
					$url_file = $preview_file;
		...
		case 'search':
			...
			parse_str( isset( $_GET['search'] ) ? trim( $_GET['search'] ) : '', $search );
			$keys = array();
			if ( $file_exists($path) && $filter != '' )
			{
				if ( $function_prefix == 's3_' )
				{
					...
					{
						if ( substr($key, -1, 1) != '/' && stripos(basename($key), $filter) !== FALSE )
						{
							$keys[] = $key;
						}
					}
				...
				else
				{
					...
					foreach ( new RecursiveIteratorIterator($it) as $key )
					{
						if ( substr($key, -1, 1) != '/' && stripos(basename($key), $filter) !== FALSE )
					    {
			...
			$response = '<ul class="nav nav-tabs nav-stacked">';
			foreach ( $keys as $key )
			{
				$response .= sprintf(
					'<li><a href="dialog.php?%s">%s</a></li>',
					htmlspecialchars( http_build_query( array_merge($search, array(
						'fldr' => substr( dirname($key), strlen($current_path) ) . '/',
						'filter' => pathinfo( $key, PATHINFO_FILENAME )
					))) ),
					htmlspecialchars( substr($key, strlen($current_path)) )
				);
			}
			if ( count($keys) == 0 )
			{
				$response = '<li>' . htmlspecialchars( trans('File_Not_Found') ) . '</li>';
			}
			$response .= '</ul>';
