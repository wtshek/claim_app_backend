    ...
    contextActions: {
      ...
      copy_url : function($trigger)
      {
        ...
        bootbox.alert(
          ...
          '<input id="url_text' + copy_count + '" type="text" style="width:80%; height:30px;" value="' + $('<div>').text(m).html() + '" />' +
      ...
      select: function($trigger)
      {
        ...
        if (external != "")
        {
          ...
          {
            var target = $('#' + external.replace(/([;&,\.\+\*\~':"\!\^#$%@\[\]\(\)=>\|])/g, "\\$1"), windowParent.document);
    ...
    makeContextMenu: function()
    {
      ...
      $.contextMenu({
        ...
        build: function ($trigger)
        {
          ...
          // extract
          // AVALADE: please do not uncomment as it is not supported
          /*
    ...
    $( "#filter-search" ).click( function() {
        ...
        $.ajax( {
            ...
            bootbox.dialog( response, [], {
                "header": path.substr( $('#current_path').val().length ) + " > " + filter
            } );
        } );
     });
  } );
...
// because of feedback and on error bahhhhh...
  ...
  apply = function(file, external)
  {
    ...
    if (is_return_relative_url == 0)
    {
      url = $('<a>').attr('href', url)[0].href;
    }
  ...
  apply_any = function(url)
  {
    ...
    else if (typeof window.parent.callback == 'function')
    {
      window.parent.callback(url);
    }
    else
    {
  ...
  function close_window()
  {
  	...
  	else
  	{
  		...
  		if (typeof parent.$(frameElement).closest('.modal').modal == "function"){
  			parent.$(frameElement).closest('.modal').modal('hide');
  		}
  ...
  apply_file_rename = function(container, name)
  {
    ...
    //preview link
    var link2 = container.find('a.file-preview-btn');
  ...
  apply_folder_rename = function (container, name)
  {
    ...
    var new_url = url.replace('fldr=' + encodeURIComponent(fldr + old_name), 'fldr=' + encodeURIComponent(fldr + name));
    link.each(function ()
    {
