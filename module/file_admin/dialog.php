...
if (isset($_GET['root'])
	&& !empty($_GET['root'])
	&& strpos($_GET['root'],'../') === FALSE
	&& strpos($_GET['root'],'./') === FALSE)
{
	$rootdir = trim(strip_tags($_GET['root']),"/") ."/";
	if ( !isset($_GET['fldr']) || strpos($_GET['fldr'], $rootdir) !== 0 )
	{
		$_GET['fldr'] = $rootdir;
	}
	$rootdir = urldecode($rootdir);
}
if (isset($_GET['fldr'])
	&& !empty($_GET['fldr'])
...
$field_id = isset($_GET['field_id']) ? trim($_GET['field_id']) : '';
			...
			init: function() {
				this.on("success", function(file,res) {
					file.previewElement.addEventListener("click", function() {
						<?php echo $apply; ?>(res,<?php echo json_encode($field_id); ?>);
					});
				});
			},
			<?php } ?>
...
<?php flush_data(); ?>
<body>
	<div id="throbber" data-loader="spinner-circle" style="margin: 3em auto"></div>
	...
	<input type="hidden" id="field_id" value="<?php echo htmlspecialchars($field_id); ?>" />
	<input type="hidden" id="type_param" value="<?php echo $type_param; ?>" />
	<input type="hidden" id="current_path" value="<?php echo $current_path; ?>" />
	<input type="hidden" id="cur_dir" value="<?php echo $current_path.$rfm_subfolder.$subdir; ?>" />
	...
	<input type="hidden" id="base_url" value="<?php echo $function_prefix == 's3_' ? '//'.$kernel->conf['cloudfront_domain'].'/' : $kernel->sets['paths']['server_url'] . str_replace('\\', '/', substr(__DIR__, strlen($kernel->sets['paths']['doc_root']))) . '/'; ?>" />
...
foreach($files as $k=>$file){
	//$dir_metadata = in_array($file, array('.', '..')) ? FALSE : ($function_prefix == 's3_' ? s3_head_object($current_path.$rfm_subfolder.$subdir.$file.'/') : $current_path.$rfm_subfolder.$subdir.$file.'/');
	$dir_metadata = in_array($file, array('.', '..')) ? FALSE : ($function_prefix == 's3_' ? s3_head_object($current_path.$rfm_subfolder.$subdir.$file.'/') : $current_path.$rfm_subfolder.$subdir.$file);
	if($file==".") $current_folder=array('file'=>$file);
	elseif($file=="..") $prev_folder=array('file'=>$file);
	elseif($is_dir($dir_metadata)){
		$date=$filemtime($dir_metadata);
		if($show_folder_size){
		...
		$metadata=$function_prefix == 's3_' ? s3_head_object($file_path) : $file_path;
		$date=$filemtime($metadata);
	...
	<div class="navbar-inner">
		...
		<div class="filters">
			...
			<div class="span6 entire types">
				...
				<label id="filter-search" title="<?php echo trans('Search'); ?>" class="tip btn btn-inverse"><i class="icon-search icon-white"></i></label>
			</div>
			</div>
		</div>
		</div>
...
<!-- header div end -->
	...
	?> <li class="active"><?php echo $b?></li><?php
		...
		<li><a href="<?php echo $link.urlencode($tmp_path)?>"><?php echo $b?></a></li><li><span class="divider"><?php echo "/"; ?></span></li>
	<?php }
	}
	?>
	...
	<li class="pull-right"><a id="refresh" class="btn-small" href="dialog.php?<?php echo $get_params.urlencode($subdir)."&".uniqid() ?>"><i class="icon-refresh"></i></a></li>
...
<?php flush_data(); ?>
	...
	<div class="span12">
		<?php if(!$file_exists($current_path.$rfm_subfolder.$subdir)){ ?>
		<br/>
...
<?php flush_data(); ?>
		...
		foreach ($files as $file_array) {
		  $file=$file_array['file'];
			if($file == '.' || (isset($file_array['extension']) && $file_array['extension']!=trans('Type_dir')) || ($file == '..' && $current_path.$subdir == $rootdir) || in_array($file, $hidden_folders) || ($filter!='' && $n_files>$file_number_limit_js && $file!=".." && stripos($file,$filter)===false))
			  continue;
...
<?php flush_data(); ?>
			...
			$file=$file_array['file'];
				...
				$src = $function_prefix == 's3_'
					? 'http://'.$kernel->conf['s3_domain'].'/'.$current_path.$rfm_subfolder.$subdir.$file
					: $kernel->sets['paths']['server_url'] . str_replace('\\', '/', substr(realpath($current_path.$rfm_subfolder.$subdir.$file), strlen($kernel->sets['paths']['doc_root'])));
...
<script>
	...
	<?php
	if(isset($files_prevent_duplicate))
	{
