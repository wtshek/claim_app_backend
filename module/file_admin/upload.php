...
$path_pos  = $current_path == '' ? 0 : strpos($storeFolder,$current_path);
$thumb_pos = $thumbs_base_path == '' ? 0 : strpos($storeFolderThumb,$thumbs_base_path);
...
if ( ! empty($_FILES))
{
	...
	if (in_array(fix_strtolower($info['extension']), $ext))
	{
		...
		$targetFileTmp = sys_get_temp_dir() . '/' . basename( $tempFile );
		...
		chmod($targetFileTmp, 0755);

		// Move to folder
		if ( !$file_put_contents($targetFile, file_get_contents($targetFileTmp)) )
		{
			unlink($targetFileTmp);
			header('HTTP/1.1 406 PUT object failed',TRUE,406);
			exit();
		}
